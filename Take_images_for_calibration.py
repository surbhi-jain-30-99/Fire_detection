...
while True:
    ...
    retR, cornersR = cv2.findChessboardCorners(grayR,(6,5),None)  # Define the number of chess corners (here 9 by 6) we are looking for with the right Camera
    retL, cornersL = cv2.findChessboardCorners(grayL,(6,5),None)  # Same with the left camera
    ...
    if (retR == True) & (retL == True):
        ...
        cv2.drawChessboardCorners(grayR,(6,5),corners2R,retR)
        cv2.drawChessboardCorners(grayL,(6,5),corners2L,retL)
